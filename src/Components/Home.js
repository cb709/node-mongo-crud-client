       ...
       if(confirm) {
        ...
        .then(data=> {
            if(data.acknowledged && data.deletedCount > 0 ) {
                const newUsers = displayUsers.filter(item => item._id !== user._id);
    ...
    <div>
      ...
      {displayUsers.length === 0 ? <h1>No Users Found</h1> : <h1>All Users</h1>}
      {
        displayUsers.map(user => {
         return   <div key={user._id}><p >{user.name} {user.email} <Button onClick={()=>handleDlelte(user)}>X</Button></p> </div>
        })
        }
    </div>
  );
